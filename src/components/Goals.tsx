            ...
            <div className="flex flex-col justify-between h-full">
                  <div >
                        ...
                        {goals && goals.length ? <ul className="font-mono tracking-tight px-4 py-2 bg-amber-200 w-max rounded-sm">
                              {goals.map((goalObj) => (
                              <li key={goalObj._id}
                              className={`flex justify-between items-center my-2 ${currentGoal === goalObj.goal? "bg-gray-900 text-white shadow-md": "hover:bg-amber-300"}`}>

                                    <button className={`cursor-pointer w-full text-start px-6 py-2 rounded-sm font-semibold text-2xl tracking-tighter`} onClick={() => setCurrentGoal(goalObj.goal)}>
                                          {goalObj.goal}


                                    </button>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="cursor-pointer size-10">
